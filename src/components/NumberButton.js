...
function NumberButton({ value, x, y, size, status, decoy, onPress, disabled }) {
  ...
  return <Animated.View pointerEvents={status==='correct'?'none':'auto'} style={[styles.wrapper,{left:x,top:y,width:size,height:size,borderRadius:size/2,transform:[{scale},{translateX}],opacity},decoy&&styles.decoyWrapper]}><Pressable onPress={()=>onPress(value)} disabled={disabled||status==='correct'} style={[styles.pressable,{backgroundColor,borderRadius:size/2,borderWidth}]} hitSlop={6}><Text style={[styles.label,{fontSize:Math.max(size*0.36,16)}]}>{value}</Text></Pressable></Animated.View>;
}
const styles=StyleSheet.create({wrapper:{position:'absolute'},decoyWrapper:{shadowColor:'#fff',shadowOffset:{width:0,height:0},shadowOpacity:0.9,shadowRadius:8,elevation:8},pressable:{flex:1,alignItems:'center',justifyContent:'center',shadowColor:'#000',shadowOffset:{width:0,height:2},shadowOpacity:0.3,shadowRadius:3,elevation:4,borderColor:'#fff'},label:{color:colors.textPrimary,fontWeight:'700'}});
