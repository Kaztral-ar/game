...
export default function GameScreen({ navigation }) {
  ...
  const { level, score, lives, registerCorrectTap, registerWrongTap, completeLevel } = useGame();
  const maxNumber = getNumberCount(level);
  const requestedButtonSize = getButtonSize(level);
  const timeLimitMs = getTimeLimitMs(level);
  ...
  const buttonSize = useMemo(() => {
    ...
    for (let columns = 1; columns <= maxNumber; columns++) {
      ...
      const sizeFromHeight = (playArea.h - Math.max(0, rows - 1) * gap) / rows;
      const candidate = Math.floor(Math.min(requestedButtonSize, sizeFromWidth, sizeFromHeight));
      if (candidate > 0) best = Math.max(best, candidate);
    }
  ...
  const decoyHighlight = useMemo(() => {
    ...
    const seed = Math.abs((nextExpected * 31) + (level * 17) + layoutSeed * 13);
  ...
  const endLevelSuccess = useCallback(() => {
    ...
    completeLevel(remainingMs != null ? remainingMs / 1000 : null);
  ...
  const handleTap = useCallback(value => {
    if (endedRef.current || isPaused) return;
    if (value === nextExpected) {
      setStatuses(p => ({ ...p, [value]: 'correct' }));
      registerCorrectTap();
      if (value === maxNumber) endLevelSuccess();
      else setNextExpected(n => n + 1);
    } else {
      setStatuses(p => ({ ...p, [value]: 'wrong' }));
      registerWrongTap();
      playWrongSound();
      setTimeout(() => setStatuses(p => {
        if (p[value] !== 'wrong') return p;
        const n = { ...p };
        delete n[value];
        return n;
      }), 300);
  ...
  return (
    <SafeAreaView style={styles.flex}>
      <View style={styles.header}>
        <HeaderItem label="LEVEL" value={level} />
      ...
      <View style={styles.playArea} onLayout={e => {
        ...
        {numbers.map(value => {
          ...
          if (!pos) return null;
          const isDecoy = value === decoyHighlight && statuses[value] == null && value !== nextExpected;
...
const styles = StyleSheet.create({
  ...
  header: { height: HEADER_HEIGHT, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 16 },
  headerItem: { alignItems: 'center' },
  headerLabel: { color: colors.textSecondary, fontSize: 11, fontWeight: '600' },
  headerValue: { color: colors.textPrimary, fontSize: 18, fontWeight: '700' },
  pauseButton: { width: 40, height: 40, borderRadius: 20, backgroundColor: colors.surface, alignItems: 'center', justifyContent: 'center' },
  pauseIcon: { color: colors.textPrimary, fontSize: 16 },
  playArea: { flex: 1, position: 'relative', marginHorizontal: 8, overflow: 'hidden' },
  overlay: { flex: 1, backgroundColor: colors.overlay, alignItems: 'center', justifyContent: 'center' },
  pauseCard: { backgroundColor: colors.surface, borderRadius: 20, padding: 24, width: 280 },
  pauseTitle: { color: colors.textPrimary, fontSize: 22, fontWeight: '700', textAlign: 'center', marginBottom: 20 },
  modalButton: { backgroundColor: colors.primary, paddingVertical: 14, borderRadius: 14, alignItems: 'center', marginBottom: 10 },
  modalButtonSecondary: { backgroundColor: colors.buttonIdle },
